apos.define('dynamic-table-widgets-editor', {
    ...
    construct : function(self,options){
        ...
        self.initTable = function(){
            ...
            var convertData = self.$data.find("input");
            convertData.val("{" + "\"data\" : " + JSON.stringify(self.rowData) + "," + "\"columns\" : " + JSON.stringify(self.columnData) + "}");
        }
        ...
        self.beforeShow = function(callback){
            return superBeforeShow(function(err){
                ...
                var rowInput = self.$row.find("input");
                var columnInput = self.$column.find("input");
                var dataInput = self.$data.find("input");
                ...
                self.$ajaxOptions.on("change" , function(e){
                    var options = JSON.parse(e.currentTarget.querySelector("input").value);
                    self.executeAjax(options);
                })

                self.$data.on("change" , function(e){
                    try{
                        var data = JSON.parse(e.currentTarget.querySelector("input").value);
                        data.data.forEach(function(value, i , arr){
                            self.rowData[i] = value;
                        })

                        data.columns.forEach(function(val, i , arr){
                            self.columnData[i] = value;
                        })
        ...
        self.afterShow = function(){
            ...
            var rowInput = self.$row.find("input");
            var columnInput = self.$column.find("input");
            // Let row change event registered first, then trigger it
