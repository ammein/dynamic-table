    ...
    construct : function(self,options){
            ...
            return superBeforeShow(function(err){
                ...
                self.$data.on("change" , function(e){
                    try{
                        ...
                        data.columns = data.columns.reduce(function(init , next, i){
                            if(next.title !== self.columnData[i].title){
                                next.sTitle = next.title;
                            }else if(next.sTitle !== self.columnData[i].sTitle){
                                next.title = next.sTitle;
                            }
        ...
        self.afterShow = function(){
            ...
            if (rowInput.val().length > 0 && columnInput.val().length > 0 && ajaxOptions.val().length === 0) {
                ...
                dataInput.trigger("input");
            }

            if(ajaxOptions.val().length > 0){
                self.$ajaxOptions.trigger("change");
                // To enable textarea auto resize
                ajaxOptions.trigger("input");
            }
        }
