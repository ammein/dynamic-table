                ...
                if(val.offsetParent !== null){
                    if ($.fn.DataTable.isDataTable($(self.$tableHTML[i]))) {
                        ...
                        if (self.EditorDataTableOptions.ajax){
                            $(self.$tableHTML[i]).on("processing.dt", function (e, settings, processing) {
                                if (!processing) {
                                    apos.schemas.dt.ajaxComplete = true;
                                } else if (processing) {
                                    apos.schemas.dt.ajaxComplete = false;
                                }
                            }).DataTable(self.EditorDataTableOptions);
                        }else{
                            // Manually
