    ...
    construct : function(self,options){
        ...
        self.executeAjax = function(options){
            ...
            if (apos.assets.options.lean){
                // Pass to load
                options.ajax.load = options.ajax.load || function(xhr){
                            ...
                            if(options.columns){
                                ...
                                if(filter[0]){
                                    ...
                                    if (getDataPos.split(".").length > 1 && getDataPos.split(".")[getDataPos.split(".").length - getDataPos.split(".").length] === property) {
                                        converData[i] = Object.assign(convertData[i] ? convertData[i] : convertData[i] = {}, convertData[i] = {
                                            [getTitle]: self.findNested(getDataPos, data[i][property])
                                        })
                                    } else {
                                        converData[i] = Object.assign(convertData[i] ? convertData[i] : convertData[i] = {}, convertData[i] = {
                                            [getTitle]: data[i][property]
                                        })
                                    }
                                }
                            }else{
                ...
                if (val.offsetParent !== null) {
                    ...
                    if (apos.assets.options.lean){
                        ...
                        try {
                            ...
                            if (!self.EditorDataTableOptions.ajax && apos.schemas.vanillaJSTable.options.ajax) {
                                delete apos.schemas.vanillaJSTable.options.ajax;
                                delete apos.schemas.vanillaJSTable.options.load;
                                delete apos.schemas.vanillaJSTable.options.content;
                            }
                            apos.schemas.vanillaJSTable = new simpleDatatables.DataTable(val, self.EditorDataTableOptions.ajax ? self.EditorDataTableOptions : {
                                data: obj
                            });
                        } catch (e) {
