apos.define("apostrophe-schemas",{
    construct : function(self,options){
        ...
        self.beforePopulate = function ($el, schema, object, callback){
            return superBeforePopulate(function(err){
                if(err){
                    return callback(err);
                }
                if ($el.find(".dynamic-table-area").length > 0) {
                    // Reset EditorDataTablesOptions
        ...
        self.afterPopulate = function ($el, schema, object, callback){
            return superAfterPopulate(function(err){
                if(err){
                    return callback(err);
                }
                if ($el.find(".dynamic-table-area").length > 0) {
                    // Recreate dynamic-table-utils
                    apos.create("dynamic-table-utils", { schemas : schema , object : object});
        ...
        function runTabs($el , $tab){
            ...
            if($el.find(".dynamic-table-area").length > 0){
                ...
                dynamicSchema.forEach(function (value, i, arr) {
