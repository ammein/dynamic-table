apos.define("apostrophe-schemas",{
    construct : function(self,options){
        ...
        function runTabs($el , $tab){
                ...
                if (val.offsetParent !== null) {
                    ...
                    if (allData.length > 0) {
                        ...
                        if(self.dt.settings.ajax){
                            // Latest Modal will always be dynamic-table-widgets-editor
                            apos.modalSupport.getLatestModal().registerTableEvent($(val).find("#dynamicTable"));
                        }
                    }
                }
            })
            ...
            if (apos.modalSupport.getLatestModal().$data.get(0).offsetParent !== null){
                apos.modalSupport.getLatestModal().executeAutoResize(apos.modalSupport.getLatestModal().$data.get(0))
            }

            if (apos.modalSupport.getLatestModal().$ajaxOptions.get(0).offsetParent !== null){
                apos.modalSupport.getLatestModal().executeAutoResize(apos.modalSupport.getLatestModal().$ajaxOptions.get(0))
            }
        }
        ...
        self.enableGroupTabs = function ($el) {
            ...
            $el.on('click', '[data-apos-open-group]', function () {
                ...
                if(self.dt.settings.ajax){
                    // Always make it false on ajaxComplete flag
