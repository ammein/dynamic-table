    ...
    construct : function(self,options){
        ...
        self.dt.rebuildDataTables = function(val){
            ...
            if (apos.assets.options.lean){
                ...
                // Destroy first
                new simpleDatatables.DataTable(val.querySelector("#dynamicTable")).destroy()

                // Merge from DataTablesJQuery
                if (self.dt.settings.columns && self.dt.settings.aoColumns) {
                ...
                try{
                    let datatable = new simpleDatatables.DataTable(val.querySelector("#dynamicTable"), {
                        data: obj ? obj : self.dt.settings.data
                    });

                    // Apply Event
                    apos.dynamicTableWidgetsEditor.registerTableEvent(datatable);
                }catch(e){
                    ...
                    $(val).find("#dynamicTable").removeClass("dataTable-table");
                }
            }else{
